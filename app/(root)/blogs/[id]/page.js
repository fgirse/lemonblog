      ...
      <div>
        <article>
          <section className="mx-auto max-w-5xl px-1 pt-24 pb-16 text-center">
            <span className="bg-[#FFF44F] mt-4 rounded-full px-3 py-1 mb-1 text-sm font-semibold text-black">
              {post?.category}
            </span>
            <p className="text-gray-200 pt-1">
              Published {moment(post?.createdAt).format("LL")}{" "}
            </p>
            <h1 className="mt-2 text-3xl font-bold text-white sm:text-5xl pb-2">
              {post?.title}
            </h1>
            ...
            >
              {post?.tags.map((tag) => (
                <div
                  key={tag}
                  className="bg-black py-1 px-2.5 md:py-2.5 md:px-4 text-white border border-white text-sm md:text-base font-medium md:font-semibold rounded-full hover:shadow-lg transition duration-3000 cursor-pointer"
                >
                  <span> # {tag} </span>
                </div>
              ))}
          ...
          <div className="mx-auto mt-10 px-2 md:px-4 max-w-screen-lg space-y-12 py-10 font-serif text-lg tracking-wide text-gray-100">
            ...
            <p className="leading-8 tracking-wider">{post?.description}</p>
          </div>
        </article>
      </div>
